...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // We need to add the credentials here 
  ...
  function addToConversationLog(card, role, content) {
    ...
    const speaker = role === 'user' ? 'User' : 'Agent (Lisa)';
...
`;
  ...
  async function startCall(currentCard) {
    ...
    const callPrompt = `
You are Lisa, an Accounts Receivable (AR) Collections Agent representing ODP Business Solutions.

⚠️ COMPLIANCE REQUIREMENTS ⚠️
- Follow each step precisely in the order outlined below; skipping or altering steps invalidates the call.
- Step 5 (Contact Verification) and Step 7 (Final Summary) are mandatory and cannot be skipped.
- ❗ Only reference invoice numbers if the customer explicitly requests them.
- ❗ If payment has been confirmed as “YES,” request only transaction details (date, method, reference), not invoice numbers.
- ❗ Whenever the customer provides a vague timeline for payment (e.g., "in a week," "in 10 days," "in 2–3 weeks"), you **must calculate an approximate date immediately**, confirm it with the customer, and record it. This is mandatory.
...
- Begin the call by asking: "Am I speaking with ${name}?"
- If the contact is incorrect → Apologize, record as “Wrong Contact,” and close the call.
- If confirmed → Proceed to the next step.

**2. Purpose of the Call**
- State: "I am reaching out regarding an outstanding balance of ${invoiceAmount}, originally due on ${formattedDue}."
...
**Scenario 4 — Confirmation of invoice numbers**
- Only if the customer explicitly requests invoice numbers → Provide them - "The invoice numbers are ${invoiceNumbers}."
